...
// On a normal host (local dev, a VPS, Render, etc.) bookings persist to a
// local JSON file. On Vercel, the filesystem outside /tmp is read-only, so
// when Vercel KV is linked (KV_REST_API_URL / KV_REST_API_TOKEN are set
// automatically once you add the integration) we persist there instead.
const USE_KV = Boolean(process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN);
const KV_KEY = 'fora:db';
let kvClient = null;
if (USE_KV) {
  // Lazy require so environments without the package (shouldn't happen once
  // installed, but keeps local dev robust) don't crash on load. Uses the
  // Redis integration's own client -- Vercel KV itself was retired and
  // existing stores were moved to Upstash Redis under the same env vars.
  kvClient = require('@upstash/redis').Redis.fromEnv();
}
...
// Google Maps link for the venue, shown as a clickable location link
// under the venue name.
const LOCATION_MAPS_URL = 'https://maps.app.goo.gl/Fg9G4Zndzf8YkVdT8';
...
async function readDb() {
  if (USE_KV) {
    const db = await kvClient.get(KV_KEY);
    return db || { bookings: [], nextId: 1 };
  }
  ensureDbFile();
