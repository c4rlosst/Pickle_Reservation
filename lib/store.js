...
// Each court can optionally override the facility's flat price_per_hour
// with its own base rate, plus a "peak" rate for a time window (e.g.
// weeknight prime time). A court missing pricePerHour entirely (older
// data, or a court that's never had its price customized) just falls
// back to the facility-wide price_per_hour -- so this never needs a
// backfill migration, it degrades gracefully.
function priceForSlot(facility, court, hour) {
  const base = Number(court.pricePerHour ?? facility.price_per_hour);
  ...
  if (hasPeak && hour >= court.peakStartHour && hour < court.peakEndHour) {
    return Number(court.peakPricePerHour);
  }
  return base;
}
...
// Admin: courts (add/remove/rename/reprice) and facility-wide operating
// hours, all together since they live in the same jsonb column + row.
// Validates the whole shape up front so a typo can't silently corrupt the
// courts list (which the entire booking grid is built from) -- this either
// saves cleanly or throws, never a partial write.
function validateSettingsInput({ openHour, closeHour, courts }) {
  openHour = Number(openHour);
  closeHour = Number(closeHour);
  if (!Number.isInteger(openHour) || !Number.isInteger(closeHour) || openHour < 0 || closeHour > 24 || openHour >= closeHour) {
    throw invalid('Operating hours must be a valid open/close range (e.g. 6 to 22).');
  }
  if (!Array.isArray(courts) || courts.length === 0) {
    throw invalid('At least one court is required.');
  }
  ...
  const cleanCourts = courts.map((c) => {
    ...
    const pricePerHour = Number(c.pricePerHour);
    if (!Number.isInteger(id) || id <= 0) throw invalid('Each court needs a valid id.');
    if (seenIds.has(id)) throw invalid('Court ids must be unique.');
    seenIds.add(id);
    if (!name) throw invalid('Each court needs a name.');
    if (!Number.isFinite(pricePerHour) || pricePerHour <= 0) throw invalid(`"${name}" needs a valid price per hour.`);

    const hasAnyPeakField = c.peakPricePerHour != null || c.peakStartHour != null || c.peakEndHour != null;
    let peak = { peakPricePerHour: null, peakStartHour: null, peakEndHour: null };
    if (hasAnyPeakField) {
      const peakPricePerHour = Number(c.peakPricePerHour);
      const peakStartHour = Number(c.peakStartHour);
      const peakEndHour = Number(c.peakEndHour);
      if (!Number.isFinite(peakPricePerHour) || peakPricePerHour <= 0) throw invalid(`"${name}" has an invalid peak price.`);
      if (!Number.isInteger(peakStartHour) || !Number.isInteger(peakEndHour) || peakStartHour < openHour || peakEndHour > closeHour || peakStartHour >= peakEndHour) {
        throw invalid(`"${name}"'s peak hours must fall within operating hours.`);
      }
      peak = { peakPricePerHour, peakStartHour, peakEndHour };
    }

    return { id, name, pricePerHour, ...peak };
  });

  return { openHour, closeHour, courts: cleanCourts };
}

async function updateSettings(input) {
  const facility = await getFacility();
  const clean = validateSettingsInput(input);

  // Don't let a court be removed while it still has live bookings/holds
  // referencing it -- those rows would be left pointing at a court that no
  // longer exists in the courts list.
  const removedIds = facility.courts
    .map((c) => c.id)
    .filter((id) => !clean.courts.some((c) => c.id === id));
  if (removedIds.length > 0) {
    ...
    const { data: active, error } = await supabase
      ...
      .in('court_id', removedIds)
      .in('status', ACTIVE_STATUSES);
    if (error) throw error;
    if (active.length > 0) {
      throw invalid('Cannot remove a court that still has active or pending bookings. Cancel or wait out those bookings first.');
    }
  }
  ...
  const { error } = await supabase
    .from('facilities')
    .update({ open_hour: clean.openHour, close_hour: clean.closeHour, courts: clean.courts })
    .eq('id', FACILITY_ID);
  if (error) throw error;
  facilityCache = null;
  return getConfig();
...
// After this many consecutive wrong passwords, the facility's admin login
// locks out for LOCKOUT_MINUTES. Deliberately reads/writes straight from the
// database (not the 30s facility cache) so a lockout can never be bypassed
// by hitting this within the cache window.
...
async function verifyAdminPassword(password) {
  ...
  const { data, error } = await supabase
    .from('facilities')
    .select('admin_password_hash, admin_login_fail_count, admin_login_locked_until')
    .eq('id', FACILITY_ID)
    .single();
  if (error || !data) throw new Error('Could not verify admin password.');

  const now = Date.now();
  if (data.admin_login_locked_until && new Date(data.admin_login_locked_until).getTime() > now) {
    const minutesLeft = Math.ceil((new Date(data.admin_login_locked_until).getTime() - now) / 60_000);
    const err = new Error(`Too many failed attempts. Try again in ${minutesLeft} minute${minutesLeft === 1 ? '' : 's'}.`);
    err.code = 'LOCKED';
    throw err;
  }

  if (!password) return false;
  const ok = await bcrypt.compare(String(password), data.admin_password_hash);

  if (ok) {
    if (data.admin_login_fail_count > 0) {
      await supabase.from('facilities').update({ admin_login_fail_count: 0, admin_login_locked_until: null }).eq('id', FACILITY_ID);
      facilityCache = null;
    }
    return true;
  }

  const nextCount = (data.admin_login_fail_count || 0) + 1;
  const update = { admin_login_fail_count: nextCount };
  if (nextCount >= LOCKOUT_THRESHOLD) {
    update.admin_login_fail_count = 0;
    update.admin_login_locked_until = new Date(now + LOCKOUT_MINUTES * 60_000).toISOString();
  }
  await supabase.from('facilities').update(update).eq('id', FACILITY_ID);
  facilityCache = null;
  return false;
}

// Admin panel's "Change password" action. requireAdmin has already verified
// the caller knows the CURRENT password before this ever runs, so this only
// needs to validate and apply the new one.
async function changeAdminPassword(newPassword) {
  const pw = String(newPassword || '').trim();
  if (pw.length < 8) throw invalid('New password must be at least 8 characters.');
...
// --- Public API --------------------------------------------------------------
// Sweeps expired holds before reading, so a slot that someone abandoned
// (closed the tab, phone died, never came back) reliably flips back to
// "Available" for the next person who looks -- not just whenever some
// unrelated action (a new hold, an admin page load) happens to trigger
// the cleanup. This app also runs as a Vercel serverless function (see
// the app.listen() comment in server.js), which rules out a simple
// setInterval background sweep -- there's no persistent process for it to
// run in -- so tying the sweep to the read path itself is what makes this
// work the same way in both local dev and production. The extra query is
// a single indexed delete against a tiny table (there are only ever a
// handful of in-progress holds at once for a small facility), so this
// isn't a meaningful cost next to the correctness it buys.
...
async function createHold(slots) {
  ...
  const rows = normalized.map((s) => ({
    ...
    price: priceForSlot(facility, getCourtFrom(facility, s.courtId), s.hour),
    status: 'pending',
...
// Admin: lock out an entire day in one action (e.g. the courts are hosting
// open play / a tournament and nothing should be bookable). Reuses
// blockSlot's own validation and the database's unique-index guarantee, one
// court/hour at a time -- slots that are already booked/pending/blocked are
// left alone and reported back as skipped rather than failing the whole
// request, since a partially-booked day is exactly when this is most useful.
async function blockDay({ date, notes }) {
  const facility = await getFacility();
  if (!isValidDate(date)) throw invalid('Invalid date.');
  const hours = hoursFor(facility);
  const reason = notes || 'Open play';
  ...
  for (const court of facility.courts) {
    for (const hour of hours) {
      try {
        await blockSlot({ courtId: court.id, date, hour, notes: reason });
        blocked += 1;
      } catch (err) {
        // Already booked/pending/blocked for this slot -- leave it as-is.
        skipped += 1;
      }
    }
  }
...
// Admin: the reverse of blockDay -- clears every admin-placed block for a
// date in one action, so undoing an open-play lockout doesn't mean deleting
// a dozen rows by hand. Only ever touches status='blocked' rows (never a
// real customer's pending/confirmed booking), whether they came from
// blockDay or a single manual block on that date.
